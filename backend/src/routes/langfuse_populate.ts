...
router.get("/assistant-completions", async (req: Request, res: Response) => {
  try {
    const testDataPath = path.join(
      __dirname,
      "../../src/assistant-completions.json"
    );
    const jsonData = await fs.promises.readFile(testDataPath, "utf8");
    const testCases = JSON.parse(jsonData);
    ...
    // Loop through each test case
    for (const [index, testCase] of testCases.entries()) {
      await evaluateAssistantCompletion(testCase);
      console.log(`Processed ${index}: ${testCase.input}`);
      if (index > 20) break;
    }
